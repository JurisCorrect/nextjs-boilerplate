...
import PaymentPanel from '../../correction/PaymentPanel' // ajuste ce chemin si ton PaymentPanel est ailleurs
...
export default async function CommentaireViewPage({ params }: Props) {
  const { data, error } = await supabase
    .from('corrections') // si ta table est différente, remplace ici
    .select('result_json')
    .eq('id', params.id)
    .single()
  ...
  const burgundyBox: React.CSSProperties = {
    background: '#7b1e3a', // bordeaux chic
    color: '#fff',
    borderRadius: 12,
    padding: '16px 18px',
